...
# Copyright 2025 Illumos Contributors
#
...
#
# (We don;t need this since we are)
...
# TODO: Autorun du command on proto/root_i386
#
MRSIZE=650M
NBPI=16384
...
#!/sbin/sh
mount -F tmpfs tmpfs /root > /dev/console
cd /root > /dev/console
...
#
sh "${MVI_DIR}"/iso/uefi.sh ${DESTDIR}

#
# paranoia, we don't want a boot archive inside the boot archive
# (Removed as without boot_archive, it doesn't boot)
#
...
ls -lsh "$ISO_NAME"
echo "Hybridizing"
sh "${MVI_DIR}"/iso/hybridize-iso.sh "${ISO_NAME}" ${DESTDIR}
ls -lsh "$ISO_NAME"
